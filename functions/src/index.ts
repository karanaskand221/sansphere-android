...
const DOCUMENT_CONVERTER_URL =
  "https://sansphere-doc-converter-rhxwm3ps6a-uc.a.run.app/convert";

const DERIVABLE_EXTENSIONS = new Set([
  "doc",
  "docx",
  "ppt",
  "pptx",
...
export const getPurchasedFileUrl = onCall(async (request) => {
  ...
  if (!exists) {
    throw new HttpsError(
      "not-found",
      "Purchased document file does not exist in Storage.",
    );
  }
  ...
  console.log("SECURE SIGNED URL CREATED", {
    uid,
    resourceId,
    bucket: bucket.name,
    storagePath: String(storagePath),
...
/**
 * Return a short-lived signed URL for the generated PDF preview.
 *
 * Preview access is deliberately separate from purchased access:
 * - never returns the original resource file
 * - requires a ready PDF derivative
 * - does not create or modify a purchase
 */
export const getResourcePreviewUrl = onCall(async (request) => {
  const uid = requireAuth(request);
  ...
  if (data.derivativeStatus !== "ready") {
    throw new HttpsError(
      "failed-precondition",
      "Preview is not ready for this resource.",
    );
  }

  const derivativePath = String(
    data.derivativeStoragePath ?? "",
  ).trim();
  ...
  const file = bucket.file(derivativePath);
  const [exists] = await file.exists();

  if (!exists) {
    throw new HttpsError(
      "not-found",
      "Preview file does not exist in Storage.",
    );
  }
  ...
  console.log("PREVIEW SIGNED URL CREATED", {
    uid,
    resourceId,
    bucket: bucket.name,
    derivativePath,
...
/**
 * Create a PDF derivative for supported academic document uploads.
 *
 * The original seller file remains untouched. The derivative is created
 * only for previewable office/text formats. PDFs remain PDFs and ZIP
 * archives remain download-only.
 */
export const createAcademicResourcePdfDerivative = onDocumentCreated(
  {
    document: "academic_vault/{resourceId}",
    database: "sanvault",
    region: "asia-south1",
    retry: true,
  },
  async (event) => {
    const resourceId = String(event.params.resourceId ?? "").trim();
    ...
    if (extension === "pdf" || extension === "zip") {
      await vaultDb.collection("academic_vault").doc(resourceId).set(
        {
          derivativeStatus: "not_required",
          derivativeUpdatedAt: FieldValue.serverTimestamp(),
        },
        {merge: true},
      );
      return;
    ...
    if (!DERIVABLE_EXTENSIONS.has(extension)) {
      await vaultDb.collection("academic_vault").doc(resourceId).set(
        {
    ...
    if (!exists) {
      throw new Error(
        `Original resource file does not exist: ${storagePath}`,
      );
    }
    ...
    const client = await auth.getIdTokenClient(DOCUMENT_CONVERTER_URL);
    const authHeaders = await client.getRequestHeaders();

    const form = new FormData();
    form.append(
      "file",
      new Blob([new Uint8Array(originalBytes)]),
      fileName,
    );
    ...
    if (!response.ok) {
      ...
      throw new Error(
        `Document conversion failed (${response.status}): ` +
    ...
    const directory =
      lastSlash >= 0 ? storagePath.substring(0, lastSlash + 1) : "";
    ...
    await derivativeFile.save(pdfBytes, {
      resumable: false,
      metadata: {
        contentType: "application/pdf",
        metadata: {
          resourceId,
          derivativeOf: storagePath,
          derivativeType: "pdf",
        },
      },
    });
    ...
    await resourceRef.set(
      {
        ...
        derivativeFileName: `${baseName}.pdf`,
        derivativeContentType: "application/pdf",
        derivativeSize: pdfBytes.length,
        derivativeUpdatedAt: FieldValue.serverTimestamp(),
      },
      {merge: true},
    );
