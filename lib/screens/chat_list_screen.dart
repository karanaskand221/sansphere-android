...
final _chatFirestore = FirebaseFirestore.instanceFor(app: Firebase.app(), databaseId: 'sanvault');
...
Future<void> startChatWithUser(
  ...
  if (!snap.exists) {
    await chatRef.set({
      ...
      'lastMessage': docTitle != null ? "Started a chat about \"$docTitle\"" : "Chat started",
  ...
  Navigator.push(
    context,
    MaterialPageRoute(builder: (_) => ChatConversationRoom(chatId: chatId, peerUid: peerUid, peerName: peerName)),
  );
}
    ...
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (chats.isEmpty) {
            return const Center(child: Text("No conversations yet — message a seller from a resource page."));
          }
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, idx) {
              ...
              final peerUid = participants.firstWhere((u) => u != myUid, orElse: () => '');
              ...
              return ListTile(
                ...
                subtitle: Text(
                  docTitle != null ? "Re: $docTitle • $lastMessage" : lastMessage,
                  maxLines: 1,
                ...
                onTap: () => Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (_) => ChatConversationRoom(chatId: chats[idx].id, peerUid: peerUid, peerName: peerName),
                  ),
                ),
              );
            },
...
class ChatConversationRoom extends StatefulWidget {
  ...
  const ChatConversationRoom({super.key, required this.chatId, required this.peerUid, required this.peerName});
...
class _ChatConversationRoomState extends State<ChatConversationRoom> {
  ...
  Future<void> _showPeerInfo() async {
    final doc = await _chatFirestore.collection('users').doc(widget.peerUid).get();
    final data = doc.data();
        ...
        content: Column(
          ...
          children: [
            ...
            Text(
              ...
              style: TextStyle(color: showPhone ? Colors.black87 : Colors.grey, fontStyle: showPhone ? FontStyle.normal : FontStyle.italic),
            ),
          ],
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [IconButton(icon: const Icon(Icons.info_outline), onPressed: _showPeerInfo)],
      ),
      body: Column(
        children: [
          Expanded(
